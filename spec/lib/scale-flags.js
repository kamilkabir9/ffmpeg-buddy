...
describe("scale flags", function () {
  ...
  it("can handle a % and -1", function () {
    assert.deepStrictEqual(scale([], "55.123%", "-1"), [
      "-vf 'scale=iw*0.55123:-1'",
    ]);
  });

  it("can handle a -1 and %", function () {
    assert.deepStrictEqual(scale([], "-1", "55.123%"), [
      "-vf 'scale=-1:ih*0.55123'",
    ]);
  });

  it("can handle two percentages", function () {
    assert.deepStrictEqual(scale([], "50%", "25%"), [
      "-vf 'scale=iw*0.5:ih*0.25'",
    ]);
  });
